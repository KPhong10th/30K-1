...
# Constants
FILE_PATH = "dec2024.csv"
SECTOR_MAP = {
    "AAPL": "Technology", "MSFT": "Technology", "NVDA": "Technology", "AI": "Technology",
...
# Rolling volatility plot with different colors
def plot_rolling_volatility(log_returns, window=30):
    """
    Plot rolling volatility for each stock with different colors.
    ...
    colors = plt.cm.tab10(np.linspace(0, 1, len(rolling_volatility.columns)))  # Use a color map

    plt.figure(figsize=(12, 6))
    for i, column in enumerate(rolling_volatility.columns):
        plt.plot(
