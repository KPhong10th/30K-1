...
stock_names = stock_prices.columns  # Assuming columns are stock names
weights = best_portfolio['weights']  # Weights from the best portfolio

# Print the weights with corresponding stock names
print("\nOptimal Weights per Stock:")
for stock, weight in zip(stock_names, weights):
    print(f"{stock}: {weight:.4f}")  # Adjust the format for desired precision
